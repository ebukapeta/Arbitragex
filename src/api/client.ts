...
export interface VerifyPriceResult {
  ...
  liveBuyPrice:     number;   // last traded price on buy exchange (= current market price)
  liveSellPrice:    number;   // last traded price on sell exchange
  liveBuyAsk:       number;   // ask price on buy exchange (reference only)
  liveSellBid:      number;   // bid price on sell exchange (reference only)
  ...
  sellFeeAmt:       number;
  wdFeeUSD:         number;
  wdFeeInCoin:      number | null;  // coin-denominated fee e.g. 0.35 MBOX — null if unavailable
  wdFeeSource:      'live' | 'fallback';
  grossProfitUSD:   number;   // sale proceeds minus invested amount (before fees)
  netProfitUSD:     number;
  netProfitPct:     number;
