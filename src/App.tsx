...
import { generateMockBalances, generateOpportunity, MOCK_PAIRS } from './data/mockData';
...
export function App() {
  ...
  const [params, setParams]                   = useState<ScannerParams>({
    ...
    minProfitPct:    0,       // 0% default — show all opportunities, user can raise it
    maxProfitPct:    50,      // 50% default — wide range to catch real signals
    minVolume24hLow: 0,       // 0 default — no volume filter until user sets one
  });
    ...
    if (BACKEND_AVAILABLE) {
      try {
        ...
        setOpportunities(prev => {
          // Keep currently-executing opportunities untouched
          const executing = prev.filter(o => o.executing);

          // For each new opportunity from the backend, preserve firstSeenAt
          // if we already have that same opportunity (same pair + buy + sell exchange)
          const merged = result.opportunities.map(newOpp => {
            const existing = prev.find(
              p => p.pair === newOpp.pair &&
                   p.buyExchange === newOpp.buyExchange &&
                   p.sellExchange === newOpp.sellExchange
            );
            return existing
              ? { ...newOpp, firstSeenAt: existing.firstSeenAt, discoveredAt: existing.firstSeenAt }
              : newOpp;
          });

          // Don't replace executing ones
          const filtered = merged.filter(o => !executing.some(e =>
            e.pair === o.pair && e.buyExchange === o.buyExchange && e.sellExchange === o.sellExchange
          ));

          return [...executing, ...filtered];
        });
      ...
      for (const pair of shuffled) {
        ...
        if (!opp) continue;
        // Backend already applied all filters — just collect what was returned
        newOpps.push(opp);
      }
  ...
  const handleExecute = useCallback((
    ...
    if (BACKEND_AVAILABLE && depositAddress) {
      setExecState({ oppId: opp.id, step: 'verifying_price', message: 'Re-fetching live prices before placing orders...' });
      api.executeTrade({
        opportunity:  opp,
        amount,
        depositAddress,
          ...
          pollStopRef.current = api.pollTradeStatus(
            ...
            (status) => {
              ...
              setExecState({
                ...
                steps:             status.steps,
                liveNetProfitUSD:  lastStep?.liveNetProfitUSD,
                liveNetProfitPct:  lastStep?.liveNetProfitPct,
                priceMovedPct:     lastStep?.priceMovedPct,
              });
            },
            (finalStatus) => {
              const lastStep    = finalStatus.steps[finalStatus.steps.length - 1];
              const netProfit   = lastStep?.netProfit ?? finalStatus.netProfit;
              const isAborted   = finalStatus.status === 'aborted';
              const isCompleted = finalStatus.status === 'completed';
              setExecState({
                oppId:            opp.id,
                tradeId:          response.tradeId,
                step:             isCompleted ? 'completed' : isAborted ? 'aborted' : 'failed',
                message:          isCompleted ? 'Trade completed!' :
                                  isAborted   ? (lastStep?.message ?? 'Trade aborted — price moved') :
                                               (finalStatus.error ?? 'Trade failed'),
                abortReason:      isAborted ? lastStep?.message : undefined,
                netProfit:        netProfit ?? undefined,
                liveNetProfitUSD: lastStep?.liveNetProfitUSD,
                liveNetProfitPct: lastStep?.liveNetProfitPct,
                priceMovedPct:    lastStep?.priceMovedPct,
                error:            !isCompleted && !isAborted ? finalStatus.error : undefined,
              });
              if (isCompleted) {
