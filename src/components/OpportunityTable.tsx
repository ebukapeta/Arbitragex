...
const ExecuteModal: React.FC<{
  ...
  // Use verified live data if available, otherwise use scanner data
  ...
  const buyFeeUSD  = verified && valid ? verified.buyFeeAmt  : (valid ? parsed * (opp.buyFee / 100) : 0);
  const sellFeeUSD = verified && valid ? verified.sellFeeAmt : (valid ? parsed * (opp.sellFee / 100) : 0);
  const wdFeeUSD   = verified ? verified.wdFeeUSD : opp.withdrawalFeeUSD;
  const netUSD     = verified && valid ? verified.netProfitUSD : grossUSD - buyFeeUSD - sellFeeUSD - wdFeeUSD;
  const netPct     = verified && valid ? verified.netProfitPct : (valid ? (netUSD / parsed) * 100 : 0);
