...
const ExecutionProgressModal: React.FC<{
  ...
  const isTerminal = execState.step === 'completed' || execState.step === 'failed';
    ...
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
      <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
        ...
        <div className={`px-5 py-4 border-b border-gray-700/60 ${
          ...
          execState.step === 'failed'    ? 'bg-red-950/40'   : 'bg-blue-950/30'
        }`}>
              ...
              <div>
                <h3 className="text-white font-bold text-sm">
                  {execState.step === 'completed' ? 'Trade Completed ✓' :
                   execState.step === 'failed'    ? 'Trade Failed'      : 'Executing Trade'}
                </h3>
                {opp && (
...
const ExecuteModal: React.FC<{
  ...
  onClose: () => void;
}> = ({ opp, onExecute, onClose }) => {
  const [amount, setAmount] = useState('');
  const parsed = parseFloat(amount);
  const valid  = !isNaN(parsed) && parsed > 0;

  const grossUSD   = valid ? parsed * (opp.profitBeforeFees / 100) : 0;
  const buyFeeUSD  = valid ? parsed * (opp.buyFee / 100) : 0;
  const sellFeeUSD = valid ? parsed * (opp.sellFee / 100) : 0;
  const wdFeeUSD   = opp.withdrawalFeeUSD;
  const netUSD     = grossUSD - buyFeeUSD - sellFeeUSD - wdFeeUSD;
  const netPct     = valid ? (netUSD / parsed) * 100 : 0;
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-2xl p-5 sm:p-6 w-full max-w-md shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="bg-gray-800/80 rounded-xl p-4 mb-3 space-y-2.5 text-sm">
          ...
          <div className="flex justify-between items-center">
            ...
            <span className="text-blue-300 font-semibold">{opp.buyExchange} @ ${formatPrice(opp.buyPrice)}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-gray-400">Sell on</span>
            <span className="text-purple-300 font-semibold">{opp.sellExchange} @ ${formatPrice(opp.sellPrice)}</span>
          </div>
          <div className="flex justify-between items-center">
          ...
          <div className="border-t border-gray-700/60 pt-2.5 grid grid-cols-3 gap-2 text-xs">
            ...
            <div className="text-center">
              ...
              <p className="text-orange-300 font-semibold">${wdFeeUSD.toFixed(3)}</p>
            </div>
          </div>
              ...
              <div className="mt-2 space-y-1">
                <div className="bg-gray-800/60 rounded-lg px-3 py-2 grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                  <div className="flex justify-between">
                    ...
                    <span className="text-yellow-400">+${grossUSD.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500">Buy fee</span>
                    <span className="text-red-400">-${buyFeeUSD.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500">Sell fee</span>
                    <span className="text-red-400">-${sellFeeUSD.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500">W/D fee</span>
                    <span className="text-red-400">-${wdFeeUSD.toFixed(3)}</span>
                  </div>
                </div>
                <div className={`rounded-lg px-3 py-2 flex justify-between text-xs font-bold ${
                  ...
                  <span className={netUSD > 0 ? 'text-green-300' : 'text-red-300'}>Est. Net Profit</span>
                  <span className={netUSD > 0 ? 'text-green-300' : 'text-red-300'}>
                    {netUSD > 0 ? '+' : ''}{netUSD.toFixed(2)} USDT ({netPct.toFixed(3)}%)
                  </span>
                </div>
              </div>
            )}
        ...
        <div className="flex gap-3">
          ...
          {canExecute ? (
            <button
              onClick={() => { if (valid) { onExecute(parsed); onClose(); } }}
              disabled={!valid}
  ...
  return (
    <div className={`bg-gray-800/60 rounded-xl p-3 border ${
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        <ElapsedTime timestamp={opp.discoveredAt} />
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-2">
        ...
        <span className={`text-[10px] px-1.5 py-0.5 rounded font-mono ${
          opp.chainCompatible ? 'bg-cyan-900/50 text-cyan-300' : 'bg-red-900/50 text-red-300'
        }`}>
          {opp.chain ?? 'No Chain'}
        </span>
...
export const OpportunityTable: React.FC<Props> = ({ opportunities, onExecute, execState, onCloseExecModal, backendMode: _backendMode }) => {
  ...
  // Row height ~44px; 5 rows + header ~44*5+44 = ~264px max for scroll container
  const ROW_HEIGHT = 44;
  const VISIBLE_ROWS = 5;
  const TABLE_MAX_H = ROW_HEIGHT * VISIBLE_ROWS;
  ...
  return (
    <>
      ...
      {executeTarget && (
        <ExecuteModal
          opp={executeTarget}
          onExecute={(amt) => { onExecute(executeTarget, amt); setExecuteTarget(null); }}
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-2xl overflow-hidden mb-4">
        ...
        <div
          ...
          style={{ maxHeight: `${VISIBLE_ROWS * 180}px` }}
        >
          {sorted.map((opp, idx) => (
            <MobileCard
              key={opp.id}
        ...
        {/* ── Desktop table (≥ sm) — scrollable body ──────── */}
          ...
          <table className="w-full text-xs" style={{ tableLayout: 'fixed', minWidth: '900px' }}>
            <thead className="sticky top-0 z-10">
              <tr className="bg-gray-800/90 border-b border-gray-700/60 text-gray-400">
                <th className="text-center py-2.5 px-2 font-semibold w-7">#</th>
                <th className="text-left py-2.5 px-2 font-semibold w-12">Base</th>
                <th className="text-left py-2.5 px-2 font-semibold w-24">Pair</th>
                <th className="text-left py-2.5 px-2 font-semibold w-20">Buy Exch.</th>
                <th className="text-right py-2.5 px-2 font-semibold w-24">Buy Price</th>
                <th className="text-left py-2.5 px-2 font-semibold w-20">Sell Exch.</th>
                <th className="text-right py-2.5 px-2 font-semibold w-24">Sell Price</th>
                <th className="text-right py-2.5 px-2 font-semibold w-20">Gross</th>
                <th className="text-right py-2.5 px-2 font-semibold w-20">Net Profit</th>
                <th className="text-center py-2.5 px-2 font-semibold w-10">W/D</th>
                <th className="text-center py-2.5 px-2 font-semibold w-10">Dep</th>
                <th className="text-center py-2.5 px-2 font-semibold w-20">Chain</th>
                <th className="text-right py-2.5 px-2 font-semibold w-20 whitespace-nowrap">
                  <span className="text-blue-300">Buy</span> 24V
                </th>
                <th className="text-right py-2.5 px-2 font-semibold w-20 whitespace-nowrap">
                  <span className="text-purple-300">Sell</span> 24V
                </th>
                <th className="text-center py-2.5 px-2 font-semibold w-16">Age</th>
                <th className="text-center py-2.5 px-2 font-semibold w-20">Execute</th>
              </tr>
            </thead>
          </table>
          {/* Scrollable tbody wrapper */}
          <div className="overflow-y-auto" style={{ maxHeight: `${TABLE_MAX_H}px` }}>
            <table className="w-full text-xs" style={{ tableLayout: 'fixed', minWidth: '900px' }}>
              <tbody>
                {sorted.map((opp, idx) => {
                  const canExecute = opp.chainCompatible && opp.withdrawalEnabled && opp.depositEnabled;
                  const blockReason = !opp.chainCompatible
                    ? 'No compatible chain between exchanges'
                    : !opp.withdrawalEnabled
                    ? `Withdrawal disabled on ${opp.buyExchange}`
                    : !opp.depositEnabled
                  ...
                  return (
                    ...
                    >
                      <td className="py-2.5 px-2 text-center text-gray-600 font-mono w-7">{idx + 1}</td>
                      ...
                      <td className="py-2.5 px-2 text-white font-semibold whitespace-nowrap w-24">{opp.pair}</td>
        ...
        {/* Scroll hint */}
        {sorted.length > VISIBLE_ROWS && (
          <div className="border-t border-gray-800/60 px-4 py-2 text-center">
            <span className="text-gray-600 text-[10px]">
              ↕ Scroll to view all {sorted.length} opportunities ({sorted.length - VISIBLE_ROWS} more)
            </span>
          </div>
        )}
      </div>
