...
const ChainSelectorModal: React.FC<{
  ...
  // Static fallback fees (used only when live fee from exchange is not available)
  const STATIC_WD_FEES: Record<string, number> = {
    TRC20: 1.00, BEP20: 0.80, SOL: 1.00, POLYGON: 1.00,
    ARBITRUM: 0.80, OPTIMISM: 0.80, BASE: 0.50, AVAXC: 1.00,
    KCC: 0.80, TON: 0.50, ERC20: 4.50,
  };

  // viableChainDetails carries real coin-denominated fees from the exchange API
  // fee field is in coin units (e.g. 0.35 MBOX) — multiply by coin price for USD
  // We don't have price here so we show coin units and let user know it's from exchange
  const viableDetails: Array<{chain: string; fee: number; withdrawEnabled: boolean; depositEnabled: boolean}>
    = (opp as any).viableChainDetails ?? [];
