...
interface Props {
  ...
  onDisconnectExchange: (exchange: Exchange) => void;
...
export const ExchangeDashboard: React.FC<Props> = ({
  balances, onTransfer, transferHistory, onConnectExchange, onDisconnectExchange,
  activeTransferId, backendMode: _backendMode,
}) => {
  ...
  const [confirmDisconnect, setConfirmDisconnect] = useState<Exchange | null>(null);
  ...
  const isExchangeConnected = (ex: Exchange) =>
    balances.find(b => b.exchange === ex)?.connected ?? false;
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-2xl p-4 sm:p-5 mb-4">
        ...
        <div className="grid grid-cols-4 sm:grid-cols-8 gap-2 mb-4">
          {balances.map((bal) => (
            <div
              ...
              onClick={() => {
                setSelectedExchange(bal.exchange);
                setPanel('connect');
                setIsUpdating(false);
                setConfirmDisconnect(null);
                setCreds({});
        ...
        {panel === 'connect' && (
          <div className="bg-gray-800/80 border border-violet-700/50 rounded-xl p-4 mb-2">
            <div className="flex items-start justify-between gap-3 mb-4">
              <div>
                <h3 className="text-white font-semibold text-sm flex items-center gap-2">
                  ...
                  {isUpdating
                    ? `Update ${selectedExchange} API Keys`
                    : isExchangeConnected(selectedExchange)
                      ? `Manage ${selectedExchange} Connection`
                      : 'Connect Exchange API'}
                </h3>
                <p className="text-gray-400 text-xs mt-0.5">
                  {isUpdating
                    ? 'Enter new keys — they will replace the existing ones on the server'
                    : isExchangeConnected(selectedExchange)
                      ? 'Keys stored securely on backend server'
                      : 'Credentials are stored securely on the backend server'}
                </p>
              </div>
            ...
            <div className="flex flex-wrap gap-1.5 mb-4">
              {EXCHANGES.map(ex => (
                <button key={ex} onClick={() => {
                  ...
                  {ex}
                  {isExchangeConnected(ex) && (
                    <span className="absolute -top-1 -right-1 w-2 h-2 rounded-full bg-green-400 border border-gray-900" title="Connected" />
                  )}
                </button>
              ))}
            </div>
            ...
            {/* Credential fields — only show when not connected OR when updating */}
            {(!isExchangeConnected(selectedExchange) || isUpdating) && (
            ...
            {isExchangeConnected(selectedExchange) && !isUpdating ? (
              ...
              <div className="space-y-3">
                {/* Status banner */}
                <div className="flex items-center gap-2 bg-green-950/40 border border-green-700/40 rounded-lg px-3 py-2.5">
                  <span className={`w-2 h-2 rounded-full flex-shrink-0 ${EXCHANGE_DOT[selectedExchange]}`} />
                  <div className="flex-1 min-w-0">
                    <p className="text-green-300 text-xs font-semibold">{selectedExchange} — Connected</p>
                    <p className="text-green-600 text-[10px]">API keys stored securely on server</p>
                  </div>
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => { setCreds({}); setPanel('none'); }}
                  ...
                  {/* Update API keys */}
                  <button
                    onClick={() => { setIsUpdating(true); setCreds({}); }}
                    className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 bg-violet-700 hover:bg-violet-600
                  ...
                  {/* Disconnect — shows confirm step */}
                  {confirmDisconnect === selectedExchange ? (
                    ...
                    <button
                      onClick={() => setConfirmDisconnect(selectedExchange)}
                      className="flex items-center gap-1.5 px-3 py-2 bg-red-900/60 hover:bg-red-700 border border-red-700/50
                        text-red-300 hover:text-white text-xs font-semibold rounded-lg transition-colors"
                      title="Permanently delete API keys from server"
                    >
                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                      </svg>
              ...
              /* Not connected OR updating — show credential form actions */
              <div className="space-y-3">
                {isUpdating && (
                  <div className="flex items-center gap-2 bg-amber-950/40 border border-amber-700/40 rounded-lg px-3 py-2 text-xs">
                    <svg className="w-3.5 h-3.5 text-amber-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                        d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/>
                    </svg>
                    <span className="text-amber-300">
                      Updating API keys for <strong>{selectedExchange}</strong> — new keys will replace the existing ones on the server
                    </span>
                  </div>
                )}
                <div className="flex gap-2">
                  ...
                  >
                    ...
                    {isUpdating ? `Update ${selectedExchange} Keys` : `Connect ${selectedExchange}`}
                  </button>
                </div>
              </div>
            )}
