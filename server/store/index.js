...
app.get('/api/health', (_req, res) => {
  ...
  res.json({
    ...
    storage:            keyStore.storageMode?.() ?? 'unknown',
  });
});
...
app.listen(PORT, '0.0.0.0', async () => {
  // Wait for keyStore to finish initialising so storage mode is known
  await keyStore.ready?.();
  const storageMode = keyStore.storageMode?.() ?? 'disk';
  const dbStatus    = storageMode === 'postgresql'
    ? '✓ PostgreSQL (permanent)'
    : '⚠ Disk only (set DATABASE_URL)';
