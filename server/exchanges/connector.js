...
// Cache of public (unauthenticated) exchange instances for scanning
const publicInstanceCache = new Map();

/**
 * Get (or create) a PUBLIC (unauthenticated) CCXT instance.
 * Used for ticker fetching on exchanges that don't need API keys for public data.
 */
export function getPublicExchangeInstance(exchange) {
  if (publicInstanceCache.has(exchange)) return publicInstanceCache.get(exchange);
  ...
  const instance = new ExchangeClass({ enableRateLimit: true, timeout: 30000 });
  publicInstanceCache.set(exchange, instance);
  return instance;
}
...
/**
 * Check live withdrawal and deposit status for a specific canonical network
 * on an exchange for USDT.
 *
 * Returns:
 *   { withdrawEnabled, depositEnabled, rawNetwork, fee, minWithdraw }
 *
 * Falls back to { withdrawEnabled: true } if the exchange API does not expose
 * per-network status (so we don't block unnecessarily on missing data).
 */
export async function checkNetworkStatus(exchange, canonicalNetwork) {
  try {
    const ex = getExchangeInstance(exchange);
    if (!ex) return { withdrawEnabled: false, depositEnabled: false, rawNetwork: canonicalNetwork };

    await ex.loadMarkets();
    const currencies = await ex.fetchCurrencies();
    const usdt = currencies?.USDT;
    if (!usdt?.networks) {
      // Exchange doesn't expose per-network info — assume enabled
      return { withdrawEnabled: true, depositEnabled: true, rawNetwork: canonicalNetwork };
    }

    // Find the network entry that normalises to our canonical network
    for (const [rawId, info] of Object.entries(usdt.networks)) {
      const { normaliseNetwork } = await import('./networkNormaliser.js');
      if (normaliseNetwork(rawId) === canonicalNetwork) {
        return {
          withdrawEnabled: info.active !== false && info.withdraw !== false,
          depositEnabled:  info.active !== false && info.deposit  !== false,
          rawNetwork:      rawId,
          fee:             info.fee ?? null,
          minWithdraw:     info.limits?.withdraw?.min ?? null,
        };
      }
    }

    // Network not found in exchange data — assume enabled (conservative)
    return { withdrawEnabled: true, depositEnabled: true, rawNetwork: canonicalNetwork };

  } catch (err) {
    console.warn(`[Connector] checkNetworkStatus error for ${exchange}/${canonicalNetwork}:`, err.message);
    // On error, assume enabled — don't block trade on transient API failure
    return { withdrawEnabled: true, depositEnabled: true, rawNetwork: canonicalNetwork };
  }
}

/**
 * Fetch real withdrawal fee for a specific coin on a specific network from an exchange.
 * The fee from exchange API is often coin-denominated (e.g. 0.35 MBOX).
 * We convert to USD using the coin's current market price.
 *
 * Returns: { feeInCoin, feeInUSD, feeCurrency, rawFee, source }
 * source: 'live' | 'fallback'
 */
export async function fetchWithdrawalFee(exchange, coin, canonicalNetwork, coinPriceUSD = null) {
  try {
    const ex = getExchangeInstance(exchange);
    if (!ex) return null;
    ...
    await ex.loadMarkets();
    const currencies = await ex.fetchCurrencies();
    const coinInfo   = currencies?.[coin] ?? currencies?.[coin.toUpperCase()];
    if (!coinInfo) return null;

    const networks = coinInfo.networks ?? {};

    // Find the network entry matching our canonical network
    for (const [rawId, info] of Object.entries(networks)) {
      if (normaliseNetwork(rawId) !== canonicalNetwork) continue;

      // Raw fee from exchange (could be in coin units or USDT)
      const rawFee = info.fee ?? info.withdrawFee ?? info.withdraw_fee ?? null;
      ...
      // Some exchanges return fee in USDT, most return it in the coin itself
      // CCXT normalises this — for USDT pairs the fee is almost always in coin units
      // We use coinPriceUSD to convert coin-denominated fee to USD
      let feeInUSD  = null;
      let feeInCoin = feeNum;
      let feeCurrency = coin;

      // If fee is suspiciously large (> $100) it's probably already in USDT for stable pairs
      // If coin is USDT itself, fee is directly in USD
      if (coin === 'USDT') {
        feeInUSD    = feeNum;
        feeInCoin   = feeNum;
        feeCurrency = 'USDT';
      } else if (coinPriceUSD && coinPriceUSD > 0) {
        feeInUSD    = feeNum * coinPriceUSD;
        feeInCoin   = feeNum;
        feeCurrency = coin;
      } else {
        // No price available — return coin fee only, caller uses fallback for USD
        feeInUSD    = null;
        feeInCoin   = feeNum;
        feeCurrency = coin;
      }
      ...
      return {
        feeInCoin:    parseFloat(feeInCoin.toFixed(8)),
        feeInUSD:     feeInUSD !== null ? parseFloat(feeInUSD.toFixed(6)) : null,
        feeCurrency,
        rawFee:       feeNum,
        rawNetworkId: rawId,
        source:       'live',
    ...
    return null; // Network not found for this coin
  } catch (err) {
    console.warn(`[Connector] fetchWithdrawalFee ${exchange}/${coin}/${canonicalNetwork}: ${err.message}`);
    return null;
  }
}
