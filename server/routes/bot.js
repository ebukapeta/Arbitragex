...
botRouter.post('/verify-price', async (req, res) => {
  ...
  try {
    ...
    // Scanner found opportunity using last traded price (current market price).
    // At execution time we use ask (buy) and bid (sell) — real market order fill prices.
    // liveBuyAsk  = what we actually pay per coin with a market buy order
    // liveSellBid = what we actually receive per coin with a market sell order
    // liveLast    = current market price (for price movement comparison vs scanner)
    const liveBuyLast   = buyTicker.last  ?? buyTicker.ask;
    const liveSellLast  = sellTicker.last ?? sellTicker.bid;
    const liveBuyAsk    = buyTicker.ask   ?? liveBuyLast;   // actual market buy fill price
    const liveSellBid   = sellTicker.bid  ?? liveSellLast;  // actual market sell fill price

    if (!liveBuyAsk || !liveSellBid) {
      return res.status(400).json({ error: 'Could not fetch live prices' });
    }

    // Profit calculated using actual execution prices (ask/bid)
    ...
    const coinsReceived  = (amount - buyFeeAmt) / liveBuyAsk;
    const grossSaleValue = coinsReceived * liveSellBid;
    ...
    // Price movement: compare live last price vs scanner's last price
    // opportunity.buyPrice is the last traded price at scan time
