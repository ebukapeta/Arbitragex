...
botRouter.post('/verify-price', async (req, res) => {
  ...
  try {
    ...
    // ── Real withdrawal fee — fetch live from exchange API ────────────────────
    // Fee is coin-denominated (e.g. 0.35 MBOX) → convert to USD using live price
    const coin = pair.split('/')[0];
    let wdFeeUSD       = WITHDRAWAL_FEES_USD[chain] ?? 1.00; // fallback
    let wdFeeInCoin    = null;
    let wdFeeSource    = 'fallback';

    try {
      const liveFee = await fetchWithdrawalFee(buyExchange, coin, chain, liveBuyLast);
      if (liveFee && liveFee.feeInUSD !== null) {
        wdFeeUSD    = liveFee.feeInUSD;
        wdFeeInCoin = liveFee.feeInCoin;
        wdFeeSource = 'live';
      } else if (liveFee && liveFee.feeInCoin !== null) {
        // Have coin fee but no price yet — use liveBuyLast to convert
        wdFeeInCoin = liveFee.feeInCoin;
        wdFeeUSD    = liveFee.feeInCoin * liveBuyLast;
        wdFeeSource = 'live';
      }
    } catch (_) {
      // Keep fallback
    ...
    return res.json({
      ...
      wdFeeUSD:         parseFloat(wdFeeUSD.toFixed(6)),
      wdFeeInCoin,      // coin-denominated fee (e.g. 0.35 MBOX) — null if not available
      wdFeeSource,      // 'live' | 'fallback'
      // Profit
...
async function executeTradeAsync(tradeId, opportunity, amount, depositAddress, minProfitPct) {
  ...
  try {
    ...
    // ── Real withdrawal fee at execution time ────────────────────────────────
    let wdFeeUSD    = WITHDRAWAL_FEES_USD[chain] ?? 1.00; // fallback
    let wdFeeInCoin = null;
    let wdFeeSource = 'fallback';
    try {
      const liveFee = await fetchWithdrawalFee(buyExchange, baseAsset, chain, liveBuyLast);
      if (liveFee && liveFee.feeInUSD !== null) {
        wdFeeUSD    = liveFee.feeInUSD;
        wdFeeInCoin = liveFee.feeInCoin;
        wdFeeSource = 'live';
    ...
    const wdFeeLabel = wdFeeInCoin !== null
      ? `${wdFeeInCoin} ${baseAsset} ($${wdFeeUSD.toFixed(4)}) [${wdFeeSource}]`
      : `$${wdFeeUSD.toFixed(4)} [${wdFeeSource}]`;

    updateTradeStep(tradeId, 'price_verified',
      `Market prices — Buy: $${liveBuyLast.toFixed(6)} | Sell: $${liveSellLast.toFixed(6)} | ` +
      `W/D fee: ${wdFeeLabel} | Net profit: $${liveNetProfitUSD.toFixed(4)} (${liveNetProfitPct.toFixed(3)}%)`,
      {
