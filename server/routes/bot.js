...
botRouter.post('/verify-price', async (req, res) => {
  ...
  try {
    ...
    // Market buy  → fills at ASK  (current best sell price on buy exchange)
    // Market sell → fills at BID  (current best buy price on sell exchange)
    const liveBuyPrice  = buyTicker.ask;   // what we pay when we market buy
    const liveSellPrice = sellTicker.bid;  // what we receive when we market sell

    if (!liveBuyPrice || !liveSellPrice) {
      return res.status(400).json({ error: 'Could not fetch live prices' });
    }

    // Recalculate profit with live prices and actual trade amount
    ...
    const coinsReceived  = (amount - buyFeeAmt) / liveBuyPrice;
    const grossSaleValue = coinsReceived * liveSellPrice;
    ...
    const netProfitUSD   = netAfterWD - amount;
    const netProfitPct   = (netProfitUSD / amount) * 100;

    // Price movement since scanner found the opportunity
    const priceMovedPct  = ((liveBuyPrice - opportunity.buyPrice) / opportunity.buyPrice) * 100;

    // Determine if still profitable enough to execute
    const stillProfitable = netProfitPct >= minProfitPct;

    return res.json({
      verified:        true,
      stillProfitable,
      liveBuyPrice:    parseFloat(liveBuyPrice.toFixed(8)),
      liveSellPrice:   parseFloat(liveSellPrice.toFixed(8)),
      ...
      wdFeeUSD:        parseFloat(wdFeeUSD.toFixed(2)),
...
async function executeTradeAsync(tradeId, opportunity, amount, depositAddress, minProfitPct) {
  ...
  try {
    ...
    // Market orders fill at: ask (buy) and bid (sell)
    const liveBuyPrice  = buyTicker.ask;
    const liveSellPrice = sellTicker.bid;
    ...
    // Recalculate net profit with live prices and real trade amount
    const buyFeePct      = buyFee  ?? TAKER_FEES[buyExchange]  ?? 0.20;
    const sellFeePct     = sellFee ?? TAKER_FEES[sellExchange] ?? 0.20;
    const wdFeeUSD       = WITHDRAWAL_FEES_USD[chain] ?? 1.00;
    const buyFeeAmt      = amount * (buyFeePct / 100);
    const coinsExpected  = (amount - buyFeeAmt) / liveBuyPrice;
    const grossSaleValue = coinsExpected * liveSellPrice;
    const sellFeeAmt     = grossSaleValue * (sellFeePct / 100);
    const netAfterSell   = grossSaleValue - sellFeeAmt;
    const netAfterWD     = netAfterSell - wdFeeUSD;
    ...
    updateTradeStep(tradeId, 'price_verified',
      `Live prices — Buy: $${liveBuyPrice.toFixed(6)} | Sell: $${liveSellPrice.toFixed(6)} | ` +
    ...
    if (liveNetProfitPct < minProfitPct) {
      updateTradeStep(tradeId, 'aborted',
        `⛔ Trade aborted — profit dropped to ${liveNetProfitPct.toFixed(3)}% ` +
        `(minimum: ${minProfitPct}%). Price moved ${priceMovedPct > 0 ? '+' : ''}` +
        `${priceMovedPct.toFixed(3)}% since scanner found this opportunity. ` +
