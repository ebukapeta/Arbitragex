...
async function validateCoinChains(coin, buyExchange, sellExchange) {
  ...
  if (viableChains.length === 0) {
    // Common chains exist but all are suspended
    const bestAnyway = selectBestNetwork(commonChains);
    const buyNet     = buyNetworks.find(n => n.network === bestAnyway);
    const sellNet    = sellNetworks.find(n => n.network === bestAnyway);
    return {
      compatible:        true,
      commonChains,
      bestChain:         bestAnyway,
      withdrawalEnabled: false,
      depositEnabled:    false,
      withdrawalFeeUSD:  WITHDRAWAL_FEES_USD[bestAnyway] ?? 1.0,
      reason:            `All common chains suspended for withdrawal/deposit`,
    };
  }
  ...
  return {
    ...
    depositEnabled:    true,
    withdrawalFeeUSD:  bestViableInfo.fee,
  };
}
...
scannerRouter.post('/scan', async (req, res) => {
  ...
  for (let i = 0; i < tasks.length; i += CHAIN_BATCH) {
    ...
    for (const result of results) {
      ...
      // Use actual coin withdrawal fee from chain validation, not USDT fee
      const withdrawalFeeUSD = chainInfo.compatible
        ? (chainInfo.withdrawalFeeUSD ?? WITHDRAWAL_FEES_USD[chainInfo.bestChain] ?? 1.0)
        : 1.0;
      ...
      opportunities.push({
        ...
        buyFee:           buyFeePct,
        sellFee:          sellFeePct,
        withdrawalFeeUSD: parseFloat(withdrawalFeeUSD.toFixed(4)),
  ...
  console.log(`[Scanner] passed: ${rejectLog.passed} | noSpread: ${rejectLog.noSpread} | failedFees: ${rejectLog.failedFees} | failedVol: ${rejectLog.failedVolume} | noChain: ${rejectLog.noChain} | noTicker: ${rejectLog.noTicker}`);
