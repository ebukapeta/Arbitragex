...
import { getExchangeInstance, getPublicExchangeInstance, fetchWithdrawalFee } from '../exchanges/connector.js';
import {
  normaliseNetwork,
  normaliseNetworks,
  getCommonNetworks,
...
const PUBLIC_ENDPOINTS = {
  'Gate.io': async (coin) => {
    try {
      const r = await fetch(`https://api.gateio.ws/api/v4/spot/currencies/${coin}`);
      if (!r.ok) return null;
      const d = await r.json();
      // Gate returns single currency object
      return { [coin]: {
        networks: {
          [d.chain ?? coin]: {
            withdraw: !d.withdraw_disabled,
            deposit:  !d.deposit_disabled,
            fee:      parseFloat(d.withdraw_fix_on_chains?.[d.chain] ?? d.withdraw_fix ?? 0),
          }
        }
      }};
    } catch { return null; }
    ...
    try {
      ...
      for (const chain of (d.data.chains ?? [])) {
        networks[chain.chainName] = {
          ...
          fee:      parseFloat(chain.withdrawalMinFee ?? 0),
        };
      }
      return { [coin]: { networks } };
    } catch { return null; }
  ...
  'BitMart': async (coin) => {
    try {
      const r = await fetch('https://api-cloud.bitmart.com/spot/v1/currencies');
      if (!r.ok) return null;
      const d = await r.json();
      const item = d?.data?.currencies?.find(c => c.currency === coin || c.id === coin);
      if (!item) return null;
      return { [coin]: {
        networks: {
          [item.network ?? coin]: {
            withdraw: item.withdraw_enabled,
            deposit:  item.deposit_enabled,
            fee:      parseFloat(item.withdraw_minfee ?? 0),
          }
        }
      }};
    } catch { return null; }
...
async function getExchangeCurrencies(exchange) {
  ...
  try {
    ...
    // Use authenticated CCXT fetchCurrencies for exchanges that require API key
    ...
    if (ex) {
      try {
        ...
        console.warn(`[Scanner] ${exchange}: fetchCurrencies failed (${err.message}) — will use public endpoint per coin`);
      }
    }
    ...
    return currencies; // may be null — getCoinNetworks will try public endpoint
  } catch (err) {
    console.warn(`[Scanner] getExchangeCurrencies failed for ${exchange}: ${err.message}`);
    return null;
  }
}

// ─── Fetch network data for ONE coin from public endpoint ─────────────────────
async function fetchPublicCoinData(exchange, coin) {
  ...
  if (cached && Date.now() - cached.fetchedAt < COIN_NETWORK_TTL) return cached.networks;
  try {
    ...
    for (const [rawId, info] of Object.entries(rawNetworks)) {
      ...
      const fee = parseFloat(info.fee ?? 0);
    ...
    console.warn(`[Scanner] fetchPublicCoinData failed ${exchange}|${coin}: ${err.message}`);
    return null;
  }
}
...
// Strategy:
//   1. Check coin-level cache (fastest)
//   2. Try authenticated fetchCurrencies cache (already fetched at scan start)
//   3. Fall back to public endpoint (for Gate.io, HTX, KuCoin, BitMart, Bitget)
async function getCoinNetworks(exchange, coin) {
  ...
  // ── Try authenticated currencies cache first ──────────────────────────────
  try {
    const currencies = currenciesCache.get(exchange)?.currencies;
    if (currencies) {
      ...
      if (coinInfo) {
        ...
        if (rawNetworks && Object.keys(rawNetworks).length > 0) {
          const networks = [];
          for (const [rawId, info] of Object.entries(rawNetworks)) {
            const canonical = normaliseNetwork(rawId);
            if (!canonical) continue;
            ...
            const fee = parseFloat(
              info.fee ?? info.withdrawFee ?? info.withdraw_fee ?? WITHDRAWAL_FEES_USD[canonical] ?? 1.0
            );
    ...
    console.warn(`[Scanner] getCoinNetworks (auth cache) failed ${exchange}|${coin}: ${err.message}`);
  }

  // ── Fall back to public endpoint ──────────────────────────────────────────
  const publicNetworks = await fetchPublicCoinData(exchange, coin);
  if (publicNetworks && publicNetworks.length > 0) {
    console.log(`[Scanner] ${exchange}|${coin}: using public endpoint (${publicNetworks.length} networks)`);
...
async function fetchAllUSDTTickers(exchange) {
  ...
  // Use authenticated instance if available, otherwise public instance
  const ex = getExchangeInstance(exchange) ?? getPublicExchangeInstance(exchange);
  if (!ex) return {};
...
scannerRouter.post('/scan', async (req, res) => {
  ...
  // Exchanges that expose public endpoints — no API key needed for ticker/network data
  // API key IS still required when the bot executes trades on these exchanges
  const PUBLIC_SCAN_EXCHANGES = ['HTX', 'KuCoin', 'BitMart', 'Bitget', 'Gate.io'];

  // An exchange is valid for scanning if:
  // (a) it has API keys connected, OR
  // (b) it is a public-endpoint exchange (no key needed for price data)
  const canScan = (ex) =>
    connectedExchanges.includes(ex) ||
    PUBLIC_SCAN_EXCHANGES.includes(ex);

  const validBuy  = buyExchanges.filter(canScan);
  const validSell = sellExchanges.filter(canScan);

  if (validBuy.length === 0 || validSell.length === 0) {
    return res.json({
      opportunities: [], scannedAt: now,
      message: connectedExchanges.length === 0
        ? 'No exchanges connected. Add API keys via the Connect API panel.'
        : `No valid exchanges selected. Connected: [${connectedExchanges.join(', ')}]. Public exchanges (no key needed): [${PUBLIC_SCAN_EXCHANGES.join(', ')}]`,
    });
  }
